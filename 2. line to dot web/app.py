...
def process_kmz(input_path, output_path):
    """
    Tempat untuk logika processing KMZ.
    Untuk sementara hanya membuat salinan KMZ.
    """
    global INTERVAL_METER
...
def process():
    ...
    if not file.filename.lower().endswith(".kmz"):
        return jsonify({
            "success": False,
            "message": "File harus berformat KMZ"
        }), 400
    
    distance = float(
        request.form.get("distance")
    )
    INTERVAL_METER = distance

    print("Jarak titik:", INTERVAL_METER, "meter")
